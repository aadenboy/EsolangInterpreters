...
For debugging purposes, you may use the `#` command to print a number literally, and `~` to toggle in and out of debugging mode.

## Implied
`interpreter.lua` is a basic runner. It simply takes the field to run as the first argument. Press enter to advance the simulation by one tick.
